...
import { Client } from '.'
import { DownloadableMessage, IGroup, ISender } from '../Types'

export class Message {
  constructor(
    private M: proto.IWebMessageInfo,
    ...
    this.type =
      (Object.keys(M.message || {})[0] as MessageType) ||
      'conversation'
    ...
    const getContent = (): string => {
      ...
      return M.message?.conversation
        ...
        ? supportedMediaType
            .map(
              (type) => M.message?.[type as 'imageMessage']?.caption
            )
    ...
    if (
      ...
      const { quotedMessage, participant, stanzaId } =
        M.message?.[this.type as 'extendedTextMessage']
          ?.contextInfo || {}
      if (quotedMessage && participant && stanzaId) {
        ...
        const getQuotedContent = (): string => {
          if (quotedMessage?.buttonsResponseMessage)
            return (
              quotedMessage?.buttonsResponseMessage
                ?.selectedDisplayText || ''
            )
          ...
          return quotedMessage?.conversation
            ...
            ? supportedMediaType
                .map(
                  (type) =>
                    quotedMessage?.[type as 'imageMessage']?.caption
                )
                .filter((caption) => caption)[0] || ''
        ...
        const { username, jid, isMod } =
          this.client.contact.getContact(
        ...
        this.quoted = {
          sender: {
            ...
            isMod: this.client.config.mods.includes(
              this.client.correctJid(participant)
            ),
            isAdmin: false
          },
  ...
  public simplify = async (): Promise<Message> => {
    ...
    return await this.client
      ...
      .then((res) => {
        ...
        if (this.quoted)
          this.quoted.sender.isAdmin = result.admins.includes(
